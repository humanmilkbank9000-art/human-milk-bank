...
<body>
    <div class="container">
        <div style="display:flex; justify-content:flex-end; margin-bottom:8px; gap:10px;" class="no-print">
            <button class="btn btn-primary" id="downloadDonationsPdfBtn" type="button" aria-label="Download PDF Report">
    ...
    <script>
        (function(){
            const btn = document.getElementById('downloadDonationsPdfBtn');
            if(!btn) return;
            btn.addEventListener('click', ()=>{
                const year = {{ (int)$year }};
                const month = {{ (int)$month }};
                const url = `/admin/reports/monthly-export-donations?year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}`;
